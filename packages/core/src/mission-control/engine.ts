...
/**
 * Executive Mission Control (docs/adr/ADR-0058-mission-control.md). The primary one-screen dashboard for
 * Alyssa: enterprise and company health, revenue, pipeline, cash flow, goals, blocked items, risks,
 * approvals, top opportunities, agent / automation / system health, AI costs, ROI, and daily priorities —
 * everything important on one screen. A read model that composes the Control Tower, the Cost CFO, and
 * Agent Observability. Deterministic, read-only (mutates nothing). Tenant-scoped.
 */
...
export class MissionControl {
  ...
  /** Assemble the one-screen snapshot from already-summarized inputs. */
  assemble(tenantId: string, input: MissionControlInput): MissionControlSnapshot {
    const i = MissionControlInputSchema.parse(input);
    ...
    // The headline: the single most pressing thing on the screen.
  ...
  private headline(i: MissionControlInput, runway: number | null): string {
