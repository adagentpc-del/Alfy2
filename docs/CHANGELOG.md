...
- Engine: registry overlay store (existing builds live as-is; seeds editable), `updatePlatformField`, `missingInfrastructure`, `createMigrationPlan`, `getMigrationPlans`; readiness check updated (15×24 verified); smoke scenario proves field completeness, switch persistence, plan content, and honest scoring. No providers removed, no real secrets stored, no live rails touched.

### Added — Alfy Forge / Divini Sovereign Cloud (Phase 1: registry + generators + trackers)
- **Module (`apps/web/assets/forge.mjs`), screens `/forge`, `/forge/new`, `/forge/projects/:id`, smoke `pnpm forge:smoke` (7 scenarios):** the private, local-first internal developer platform, Phase 1 per the MVP order. New Platform Wizard (12 questions) → 24 automatic steps with **honest statuses**: generated artifacts (project record, folder structure, PRD/TECH_SPEC/BUILD_PLAN/SECURITY_CHECKLIST/COST_CONTROL_PLAN/CHANGELOG, app scaffold, Drizzle schema — WebAuthn users, RLS-paired migrations, Divini-Pay-not-local-payment-tables rule — env template with vault references, storage/auth/email configs, deployment service, backup policy), execution packets for real infrastructure (repo/db/local preview/first commit), and remote deploys parked as **deploy-class approvals**. Answers condition the pipeline (skips are labeled, never faked).
- **Secrets vault:** reference-only (`vault:`/`op://`/`env:`/`bw://`/`keychain:` accepted; raw secret material rejected and the rejection audited); **AI exposure off by default** — agents get a secret only via an explicit reasoned grant, audited.
- **Platform registry:** all 12 existing build tracks (Alfy2 → Divini Pay) with SaaS dependency maps, a sovereign replacement map (GitHub→Forgejo P2, Vercel/Render→Coolify P3, Supabase→self-hosted P4, S3→MinIO P5, Resend→Postal/Plunk P6, Stripe→Divini Pay rails), **migration-readiness scoring that only rises when a replacement phase is actually live**, and per-platform manual task generation.
- **17 dashboard sections** (Command Center → Payment Infrastructure) with truthful phase labels; **14-agent infrastructure desk** (Chief Infrastructure Architect → Source of Truth Agent, 8 dossier fields each, chained to the CTO cabinet seat); readiness gains an Alfy Forge section (now 46 checks). Implementation plan: `docs/ALFY_FORGE_IMPLEMENTATION_PLAN.md` (20 sections, incl. what-not-to-build-yet: k3s, Mailu, LocalStack, wallet). No proprietary code/UI copied — original Divini UX over open-source/self-hosted tools.

### Added — Divini Pay: privacy-first payment OS (Phase 1 Lite mock + Phase 2 marketplace flows + Phase 3 wallet lock)
- **Engine (`apps/web/assets/divini-pay.mjs`, screen `/pay`, smoke `pnpm pay:smoke`):** non-custodial payment layer for the whole portfolio. Fee engine with three rails (ACH 0.8% capped $5 / card 2.9%+30¢ / instant 1%+25¢) and an honest comparator; consent-required onboarding with KYC/KYB *readiness* fields (data minimization); tokenized instruments only — raw card/account numbers are rejected at the boundary; W-9 workflow with vault-refs only (TIN/SSN-shaped input rejected) and **W-9-before-vendor-payout enforced**; append-only double-entry ledger whose accounts are tokens/system accounts only (PII structurally separated, verified on the privacy dashboard); revenue split rules per platform (5% fee, 30%-of-fee referral default) with smoke-verified math; payouts and refunds **always park as `move_money` approvals** — two-key: a role requests, Alyssa's token releases; milestone-based releases, dispute holds (open→hold, resolve→restore), vendor risk + velocity limits ($25k/day default → risk queue), RBAC (owner/finance_admin/ops/auditor/viewer, denials audited), owner-only overrides requiring substantive reasons, CSV reconciliation export.
- **Compliance posture (binding, `docs/DIVINI_PAY_COMPLIANCE_CHECKLIST.md`):** explicitly not designed to evade lawful oversight — AML/KYC, sanctions hooks, Nacha authorization records, tax reporting, consumer protection, and money-transmitter analysis are design inputs; privacy means no data selling / no ad targeting / minimization. 15 security + 10 privacy requirements dispositioned; retention schedule drafted.
- **Phase 3 wallet:** fully designed (features, licensing tracker, sponsor-bank readiness) and **hard-locked in code** — `walletOperation()` throws until CCO+BSA/AML sign-off, sponsor bank, and the MTL decision clear.
- **Payments desk:** 12 agents (Chief Fintech Architect → Tax Reporting) under the CFO Agent, each with mission/responsibilities/inputs/outputs/decision rules/escalation triggers/compliance warnings/KPIs (`docs/DIVINI_PAY_AGENTS.md`); readiness gains a Divini Pay section (now 40 checks) verifying the surface, the ACH-first comparator, the wallet lock, the desk, RBAC, and the lawful-oversight posture.

### Added — Enterprise readiness verification + R&D bench (ASI-Arch intake)
- **Readiness layer (`apps/web/assets/readiness.mjs` + `/readiness` screen + `pnpm readiness:smoke`):** 34 machine-run checks across six sections — Hierarchy & reporting (Alyssa DelTorre seated as Founder/CEO/Chairwoman; **Alfy2 seated as Chief Operating Intelligence System**; 16/16 exact cabinet titles; 10/10 portfolio agents, each reporting agent→Alfy2→Alyssa with a resolving cabinet sponsor and company back-link) · Knowledge/skills/roles (all 26 dossiers complete, KPIs with targets/trends, owned workflows, statuses + next actions) · Guardrails & approvals (valid authority tiers, explicit approval requirements, Approval Center + live-gate bridge + studio gate submitters present, factory action classes correct) · Functional layers (10 service + 10 factory + 20 studio functions, 22 studio data objects, 11 companies with OS payloads, brain-graph integrity, report composition) · Avatar governance (consent, allowlist, hash binding, output review, usage log) · R&D bench. The screen renders the chain-of-command chart and live check results; the smoke includes a tamper test (breaking a reporting chain flips the verdict to NOT READY).
- **R&D department intake — GAIR-NLP/ASI-Arch** (`docs/RND_ASSET_ASI_ARCH.md` + `RND_ASSETS` registry, surfaced on `/readiness`): verified from the live repo (Apache-2.0, multi-agent pipeline, MongoDB+FAISS memory, RAG cognition base, ~1.2k stars). Verdict **NEEDS REVIEW → evaluate in sandbox** — it executes generated code and trains models, so binding guardrails: never in Alfy2's runtime, isolated sandbox only, budget approval before GPU spend, findings return as provenance-tagged knowledge items. Owner: CTO Agent; steward: CKO Agent; pattern-mapping table shows how its loop validates Alfy2's own research-loop design (with approval gates as our deliberate difference).

### Added — Orchestrator v0 (daily-brief job) + luxury UI polish + design-system docs
- **Orchestrator v0 (`services/orchestrator` — was a README stub):** `scheduler.ts` — cadence jobs idempotent per (job, period) with period keys (daily/hourly/weekly), bounded retries (default 3) → `period_exhausted` alert event, run-ledger port (in-memory now, Pg later); `jobs/daily-brief.ts` — calls `GET /mission-control/brief` with the API token once per day; `main.ts` — interval runner, JSON logs, `ORCH_PAUSED` kill switch, boot-safe exit when env is absent. Added to root `tsc -b`; smoke `pnpm orch:smoke` (5 scenarios incl. no-infinite-retry proof).
- **Executive strip everywhere:** every screen (command center, cabinet, agent dossier, portfolio, company OS, approvals, factory, studio, episode workspace, avatar, weekly report) now opens with the nine executive facts — status, priority, owner/agent, next action, blocked, approvals needed, revenue relevance, last updated, and a gold-washed **recommended executive decision** — computed from live state, empty cells dropping silently.
- **Approval drawer:** clicking any approval title slides in a full-context drawer (ask/impact/evidence/requester/business/decision trail + gate-mechanics footnote) with working Approve/Deny; Esc/scrim close; decisions land in the same service + action log as inline buttons.
- **Motion system (Divini brand kept):** staggered card entrances (450ms rise, 40ms stagger), animated gold nav indicator, button lift, drawer slide, shimmer skeletons for async regions (live gate queue), serif-italic ✧ empty states that say what to do next — all disabled under `prefers-reduced-motion`.
- **Design-system docs (6, indexed in DOCUMENTATION.md):** DESIGN_SYSTEM (tokens/type/spacing/motion/hard rules), UI_COMPONENT_GUIDE (component contracts), EXECUTIVE_DASHBOARD_COMPONENTS (strip + bindings + decision surfaces), STATUS_CHIP_SYSTEM (one state vocabulary), PORTFOLIO_COMPANY_VIEW (company visual contract), ENTERPRISE_NAVIGATION_STRUCTURE (IA + route map + growth policy).

### Added — Media Studio + AI Avatar command layer + Divini Group branding
- **Media Studio (`/studio`, `apps/web/assets/media-studio.mjs`):** the professional podcast/video workflow as a command layer — Riverside-style recording/editing stays external; Alfy2 owns the pipeline, metadata, gates, and export packets. Modules: series planner, episode workspace (research board → hook bank → outline → recording prep → transcript import → deterministic clip detection/planner → publishing packet (titles/description/thumbnail brief) → monetization review (claim scanner: medical/financial/legal patterns) → repurposing board → publishing jobs → avatar queue). **Five gates enforced in code** (functions throw until approved): concept before production, talking points before recording, clips before publishing, title/thumbnail/description pack before publishing, monetization-sensitive claims. 22 data-object collections per the studio spec.
- **AI Avatar command layer (`/studio/avatar`):** avatar + voice profiles with a **documented consent record** (scope, revocation, document ref), approved-use-case allowlist enforced at script creation, **script approvals bound to a deterministic content hash** (any edit invalidates the token), vendor export packet (persona/voice/consent refs + 5 guardrails + `ai_generated` disclosure — no assets in repo, references only), output review gate before publish, and an **append-only usage log** covering every step. Rules from docs/AI_AVATAR_ENGINE_SPEC.md enforced: authorized likeness only, no third-party cloning, no deceptive impersonation, every job logged, every output flagged.
- **Divini Group branding:** theme tokens reskinned from navy to the Divini palette (cream `#f8f5ee`, deep emerald `#143829`/`#1e4d3b`, gold `#a8842c`/`#c2a05a`) with Playfair Display (Didone) + Montserrat and safe fallbacks; grid overflow hardening. Spec token table updated in EXECUTIVE_DASHBOARD_SPEC.md.
- Smoke `pnpm studio:smoke` (7 scenarios, every gate proven to block first); full browser E2E: episode from concept → five approval round-trips → "Ready to publish", avatar script → hash approval → job → vendor packet → output review → published.

### Added — Four creation factories + Approval Center live wiring
- **Creation modes (`apps/web/assets/factories.mjs` + `/factory` screens):** Create a Company (Venture Factory, 14 sections: profile/model/mission/positioning/ICP/offer suite/revenue model/departments/role map/SOP library/risk register/legal-admin checklist/launch checklist/KPI dashboard), Create a Software Platform (Build Factory, 12 sections incl. **Fable/Claude/OpenClaw prompt packets**, data schema, QA plan, deployment checklist), Create a GTM Campaign (GTM Factory, 13 sections: thesis→segment→positioning→funnel→copy→magnet→nurture→cold outreach→social calendar→podcast angles→CRM stages→KPI targets→30/60/90), Create a Media/Podcast Asset (Media Studio, 17 sections incl. hook bank, clip plan, repurposing 42-map, **AI avatar job placeholder**).
- **Mechanics:** deterministic editable templates (no AI calls; missing inputs become explicit "TO ANSWER" prompts, extracted as an Open Questions panel); every edit creates a packet version; `submitPacketForApproval()` routes the go-ahead to the Approval Center with the right action class (media→publish_public, gtm→send_message, company/software→other) while execution steps keep their own gates; exports: `exportPacketToMarkdown` (download), `exportPacketToObsidian` (YAML frontmatter + tags + business wikilink), `exportPacketForAgent` (JSON with guardrails + unresolved questions, target fable/claude/openclaw). Data objects: factory_requests, per-kind packets, generated_assets, packet_versions, packet_approvals. Smoke `pnpm factorymodes:smoke` (6 scenarios).
- **Live wiring (Day-2 slice):** services.mjs gains a live bridge (`liveEnabled/getLiveApprovals/decideLiveApproval/getLiveMissionControl`); the Approval Center renders the **real gate queue** with working approve/deny (consumes the one-time-token gate), and command-center revenue + needs-you tiles go live on connect. Mock remains the instant fallback.

### Added — Enterprise Command Center v2: the operating model made visible (functional preview layer)
- **SPA (build-free, ADR-0127):** `apps/web` restructured into `index.html` shell + `assets/theme.css` + `assets/data.mjs` + `assets/services.mjs` + `assets/app.mjs`. Real URL routes via a Vercel rewrite (`vercel.json`); zero frontend dependencies preserved.
- **Screens (8):** `/command-center` — next-best-action banner + the 8 dashboard cards (business status, revenue priority, active campaigns, pending approvals, blocked workflows, agent recommendations, weekly summary, brain preview) + preserved live `Connect` hook; `/agents` + `/agents/:id` — the Enterprise Agent Cabinet (16 executive seats incl. Chief Strategy/Venture Architect/…/Data & Analytics) and 10 portfolio agents, each with a full 13-field dossier (mission, responsibilities, authority level, approval requirements, owned workflows, KPIs w/ trends, status, next action, risks, cadence, linked business, registry ref); `/portfolio` + `/portfolio/:id` — 11-company roster and the Company OS Viewer (departments active, SOP coverage, asset checklist, playbooks, connected tools, open/blocked workflows, weekly focus, per-company approvals + logs); `/approvals` — Approval Center per docs/APPROVAL_CENTER_SPEC.md (money/contracts first, approve/deny with logging, decided trail); `/reports/weekly` — weekly operating report generator composing live from state; `/brain` — Obsidian-style knowledge graph (44 nodes: founder/companies/agents/knowledge domains; interactive canvas, click-through).
- **Service layer:** `services.mjs` mirrors the future API surface — getAgents/getAgentById/getPortfolioCompanies/getCompanyOS/createApprovalRequest/approveRequest/rejectRequest/createActionLog/generateWeeklyOperatingReport/getExecutiveDashboardSummary — with an injectable store (localStorage in browser, memory in tests). Node smoke `pnpm ui:smoke` (7 scenarios) green. Parent-company revenue roll-up excluded from portfolio sums (no double-count).
- Mock-only by design: no live integrations; decisions persist in the browser only; every screen carries the preview banner.

### Added — Enterprise OS layer: build audit + 21 enterprise docs + GTM Factory + dashboard retheme
- **Build audit** (`docs/BUILD_AUDIT_CURRENT_STATE.md`): full point-in-time audit — existing/missing/broken/duplicated modules, UI/data/integration state, risk ranking, fastest demo path. Reconciles the stale counts in `ALFIE_MASTER_CONTROL.md` (notice added there) against `AI_PROJECT_OS/10_CURRENT_STATE.md`.
- **Enterprise docs (21)**, all registered in `docs/DOCUMENTATION.md` §2: the spine (`ALFY2_ENTERPRISE_OPERATING_SYSTEM.md` — fifteen systems mapped to built code; product name settled as **Alfy2**), the agent layer (`ENTERPRISE_AGENT_CABINET`, `AGENT_TITLE_REGISTRY` — doc mirror of the 78 seeded role cards, `AGENT_AUTHORITY_MATRIX`, `AGENT_REPORTING_STRUCTURE`, `AGENT_OPERATING_CADENCE`, `AGENT_KPI_SYSTEM`), the portfolio + factory layer (`PORTFOLIO_COMPANY_OS` — canonical roster incl. the four previously undocumented companies, `VENTURE_FACTORY_SPEC`, `BUILD_FACTORY_SPEC`, `GTM_FACTORY_SPEC`, `REVENUE_ENGINE_SPEC`, `MEDIA_STUDIO_SPEC`, `AI_AVATAR_ENGINE_SPEC` — net-new, `ENTERPRISE_SETUP_ENGINE_SPEC`, `APPROVAL_CENTER_SPEC`, `KNOWLEDGE_BRAIN_SYNC_SPEC` — net-new sync design, `AUTOMATION_ORCHESTRATION_SPEC`, `EXECUTIVE_DASHBOARD_SPEC` — reconciles the three dashboard concepts), and `FIVE_DAY_COMPLETION_PLAN.md`.
- **GTM Factory** (the one enterprise system missing as a module): `packages/shared/src/contracts/gtm-factory.ts` + `packages/core/src/gtm-factory/engine.ts` + `scripts/gtm-factory-smoke.mts` (`pnpm gtm:smoke`, 6 scenarios). One offer → complete launch plan (ICP, positioning, channel plans owned by registry agent titles, asset checklist, warm-up→launch→follow-through calendar, execution packets, measurement); **every external step carries its approval action class** (`send_message`/`publish_public`); deterministic, tenant-scoped, in-memory (house pattern; persistence when the API exposes it).
- **Dashboard retheme** (`apps/web/index.html`): luxury command-center tokens per `docs/EXECUTIVE_DASHBOARD_SPEC.md` — ivory base, deep-navy structure, luminous gold accents, serif display headers; brand corrected Alfie2→**Alfy2**; hardcoded date now dynamic. Values-only CSS changes; `connect()`/live-fetch logic untouched. tsc -b green; `gtm:smoke` + `aiorg:smoke` + `contentfactory:smoke` + `apiapproval:smoke` green.
...
- **Capital Allocation** (`CapitalAllocationEngine` + capital_accounts/_allocations/_runway, migration 0243): Profit-First split across 9 buckets (sums exactly to inflow), runway + mode (emergency/growth/profit_first). **Recommend-only — Alfie never moves money** (approved=false always).
- `services/api` routes: `GET /revops/brief`, `GET /revops/fastest-path`, `POST /decisions/evaluate`, `GET /decisions`, `POST /decisions/:id/decide`, `POST /capital/allocate`, `POST /capital/runway`, `GET /capital/accounts`. Gateway smoke covers all (9 scenarios). Full tsc -b green, 650 pytest, DB now 245 tables, 0 without RLS.
